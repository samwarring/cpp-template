...
function(rename_file OLD_PATH NEW_PATH)
  ...
  execute_process(
    COMMAND "${GIT_EXECUTABLE}" mv "${CMAKE_CURRENT_LIST_DIR}/${OLD_PATH}"
            "${CMAKE_CURRENT_LIST_DIR}/${NEW_PATH}" COMMAND_ERROR_IS_FATAL ANY)
endfunction()

# Replaces occurrances of a string in a file
function(replace_in_file FILE_PATH OLD_STRING NEW_STRING)
  message(
    "Replace string in ${FILE_PATH}: \"${OLD_STRING}\" -> \"${NEW_STRING}\"")
  if(${DRY_RUN})
    return()
  endif()
  file(READ "${CMAKE_CURRENT_LIST_DIR}/${FILE_PATH}" OLD_FILE_CONTENTS)
  string(REPLACE "${OLD_STRING}" "${NEW_STRING}" NEW_FILE_CONTENTS
                 "${OLD_FILE_CONTENTS}")
...
function(remove_file FILE_PATH)
  ...
  execute_process(
    COMMAND "${GIT_EXECUTABLE}" rm "${CMAKE_CURRENT_LIST_DIR}/${FILE_PATH}"
...
function(update_readme)
  ...
  set(DESCRIPTION_PATTERN
      "<!-- begin new description -->.*<!-- end new description -->")
  file(READ "${CMAKE_CURRENT_LIST_DIR}/README.md" OLD_FILE_CONTENTS)
  string(REGEX REPLACE "${DESCRIPTION_PATTERN}" "${NEW_DESC}" NEW_FILE_CONTENTS
                       "${OLD_FILE_CONTENTS}")
...
function(stage_changes)
  ...
  execute_process(COMMAND "${GIT_EXECUTABLE}" add --all COMMAND_ERROR_IS_FATAL
                          ANY)
endfunction()
...
replace_in_file(test/CMakeLists.txt cpp_template_test
                ${NEW_NAME}${NAME_SEP}test)
replace_in_file(test/CMakeLists.txt cpp_template ${NEW_NAME})
replace_in_file(include/cpp_template.h INCLUDED_CPP_TEMPLATE
                "INCLUDED_${NEW_NAME_MACRO}")
...
replace_in_file(test/cpp_template_test.cpp cpp_template_test
                "${NEW_NAME_UNDERSCORES}_test")
